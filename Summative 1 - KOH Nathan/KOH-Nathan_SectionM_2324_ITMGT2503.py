...
for i in jsondata:
    jsondata[i] = str(jsondata[i])
    jsondata[i] = jsondata[i].upper()

    for x in unnecessary_text:
        jsondata[i] = jsondata[i].replace(x, "")

    for y in unnecessary_punctuation:
        jsondata[i] = jsondata[i].replace(y, " ")
...
for i in jsondata:
    list = jsondata[i].split(" ")
    eligible_words = []
    for x in list:
        if len(x) >= 3:
            eligible_words.append(x)

    for y in range(len(eligible_words)):
        if eligible_words[y] in wordcount_dictionary.keys():
            count = wordcount_dictionary[eligible_words[y]] + 1
            wordcount_dictionary.update({eligible_words[y]:count})
        else:
            wordcount_dictionary.update({eligible_words[y]:1})
...
optimal_R = 0
optimal_C = 0
gold = (1000*optimal_F + 600*optimal_R + 750*optimal_C) <= 18000
lumber = (500*optimal_F + 750*optimal_R + 300*optimal_C) <= 12000
food = (50*optimal_F + 45*optimal_R + 75*optimal_C) <= 1500
NNC_F = optimal_F >= 0
NNC_R = optimal_R >= 0
NNC_C = optimal_C >= 0

while (1000*optimal_F + 600*optimal_R + 750*optimal_C) <= 18000 and (500*optimal_F + 750*optimal_R + 300*optimal_C) <= 12000 and (50*optimal_F + 45*optimal_R + 75*optimal_C) <= 1500 and optimal_F >= 0 and optimal_R >= 0 optimal_C >= 0:
    optimal_F += 1
    print(optimal_F)
...
def Collatz_winner(number_list):
    ...
    for x in number_list:
        ...
        length_collatz.update({x:len(sequence)})
        max_collatz.update({x:max(sequence)})
        length_list.append(len(sequence))
        max_list.append(max(sequence))
        keys.append(x)


    key = length_list.index(max(length_list))+start

    if max_list[length_list.index(max(length_list))] == max(max_list):
        return keys[length_list.index(max(length_list))]
    else:
        return "No winner"
